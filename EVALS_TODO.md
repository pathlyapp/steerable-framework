...
**16 道红题里只有 5 道拿到了同配置的第二个样本，其中 4 道变绿，只有 `sanitize-git-repo` 三次都红。**（拿到第二样本的恰好是跑得最快的 5 道，因为慢的分片在取消前没跑完——这是选择性,不是随机抽样，但方向足够清楚。）
...
- [ ] **2.5.8** 持久性指令 + 门禁判据打包成一个 arm 测。单次全量分辨不了这个量级，按本文档自己的功效表要走配对 A/B；`flaky` 的 25 题是按切流那一轮的翻面集选的，0.8202 之后翻面集已经变了，要用三次全量的同题对照重建
...
- [ ] **2.5.11** 压每步文本量（红题 3.3 min/次调用 vs 绿题 0.8）。切流式"打断"已证伪，`max_tokens` 未设过，需配对 A/B
- [ ] **2.5.12** 逐题翻面率 13.4% 已实测，**所有靶子选择都要按均值和多样本来，不能按单跑的红题名单**。`flaky` split 要用同配置的多次样本重建（现有 3 次里只有 2 次同配置，且第二次只完成 67 道）
- [ ] **2.5.13** 0.8202 复现性：在 `ci/evals-stability-27d521a`（钉死 `27d521a`，即产出 0.8202 的那个 commit，**不含 `20a854d` 的门禁与提示词改动**）连跑 3 次全量，与 0.8202 合成 4 个样本，直接测均值的复现区间而不是靠反推。派发 [33530806570](https://github.com/pathlyapp/steerable-framework/actions/runs/33530806570)、[33530856872](https://github.com/pathlyapp/steerable-framework/actions/runs/33530856872)，第三次由 `/tmp/stability-runs.sh` 在第一次结束后接上——workflow 的并发组只容得下 1 个运行中 + 1 个待队列，第三次提前派发会把待队列那个取消掉。三次串行，约 9–12 h
