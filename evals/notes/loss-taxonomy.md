...
| Live-lock pre-wrap + keep-required | **No. Keep off** | GHA [33976774219](https://github.com/pathlyapp/steerable-framework/actions/runs/33976774219) (`de45915`). 20 paired, p=1.0000, CI includes 0. 7 B fires, 6 reward 0. Mechanism `circuit-fibsqrt` A 3/3 vs B 2/3 |
...
| `validator: self_critique` | **Next** (rewire then A/B) | Headless now passes the TB instruction as `user_question`. [34003565140](https://github.com/pathlyapp/steerable-framework/actions/runs/34003565140) on `ddce50c` was cancelled: empty question made claimed/eager-deferred a no-op. `ChainHooks` prefers `retry` over `narrate` |
