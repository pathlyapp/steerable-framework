...
_SYSTEM = (
    ...
    "check, not a rewritten metric. Token counts must use that tokenizer's "
    "special-token and concatenation settings (do not add BOS/EOS unless "
    ...
    "PNG/JPEG files are pixels, not UTF-8: read_file returns an ASCII "
    "preview for 8-bit PNG (square images also get a rank/file 8x8 "
    "brightness grid); decode with Python (PIL/numpy) or ffmpeg "
    "for exact pixels. "
