...
| `regex-chess` | Live-lock | Catalog 33497477757 trial `Rbko3vm`: 4 tools then `wrap_up_named_output` at round 2, then **9** `tool_choice=required` pre_steps with **0** tools, then `[hard_timeout]`. spiral-red [33966527336](https://github.com/pathlyapp/steerable-framework/actions/runs/33966527336) 0/3, livelock on all three; after WRITE_NOW the loop stopped requiring and they kept designing | **Wrap-up livelock fired and did not convert.** Do not score this task. Keep-required until wrap-up quality gates pass is on `de45915`; livelock detector stays default-off (3b [33976774219](https://github.com/pathlyapp/steerable-framework/actions/runs/33976774219)) |
| `sanitize-git-repo` | Over-rewrote git history | Catalog 33497477757 `B7ux4TG`: secret tests passed; then `git filter-branch` + `gc --prune` left `d6987af… missing`. Oracle only `sed`s the working tree. CC GLM `bTtfUKu` passed all 3 tests. `_SYSTEM` SHA clause was not in that run | **Landed**: `_SYSTEM` + bash gate refusing `filter-branch` / `git-filter-repo` / `gc --prune` in the command or in an invoked helper script unless the instruction requires a rewrite (`rewriting history`, or recover-then-purge so it cannot be found anywhere). Official git-leak-recovery is that exception (catalog 69 pruned dangling objects). 69-run also probed `which git-filter-repo` (not a veto). `git-multibranch` 4/4 did not run those commands. Measure on catalog |
...
| Live-lock pre-wrap + keep-required | **No. Keep off** | GHA [33976774219](https://github.com/pathlyapp/steerable-framework/actions/runs/33976774219) (`de45915`). 19 paired, p=0.7266, CI includes 0. 7 B fires, 6 reward 0. Mechanism `circuit-fibsqrt` A 3/3 vs B 2/3 |
| Native PNG/JPEG on `read_file` (`STEERABLE_READ_IMAGES=1`) | **Next** | CC Read sends pixels; we ASCII-preview. GLM-5.3-Flash is multimodal. Mechanism: `code-from-image`. OpenAI tool messages are text-only, so pixels follow as a user turn. The image arm also swaps the `_SYSTEM` OCR sentence |
| `validator: self_critique` | After image-read | Discipline retry + grounding. `ChainHooks` now prefers `retry` over `narrate`, so wrap-up write-forcing is not stolen. Measure after livelock |
| CC-align prompt (`STEERABLE_PROMPT_CC_ALIGN=1`) | After self_critique | Persist-if-long + grep/glob. Tool-description facts landed as defaults |
...
| Instruction example stdout (`running … should output`) | Landed | Compiles sibling `{bin}.c` when the binary is missing. Wrap-up also runs the check (69 never completed). Does **not** guarantee a correct circuit after they already know 104 (70). 3b livelock did not convert the remaining circuit miss |
