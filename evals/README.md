...
`--split cheap-12` is the live weekly gate (12 ids). `--split failed-prev` reruns the previous catalog's failures (~41 ids, 24 shards) for harness iteration. `--split catalog` is all 89; GitHub Actions runs it via `Evals weekly` `workflow_dispatch` with split `catalog` (16 shards).
