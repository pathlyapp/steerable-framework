...
`--split cheap-12` is the live weekly gate (12 ids). `--split failed-prev` reruns the previous catalog's failures (~41 ids, 4 shards) for harness iteration. `--split catalog` is all 89; GitHub Actions runs it via `Evals weekly` `workflow_dispatch` with split `catalog` (8 shards).
