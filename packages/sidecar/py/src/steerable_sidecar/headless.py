...
_SYSTEM = (
    ...
    "poll until the login prompt or desktop is actually there. If the "
    "instruction names a monitor socket for keyboard, send a key and "
    "confirm the framebuffer changes. "
    ...
    "sidecar. If it shows an example input and the exact output that must "
    "be produced, run that example and keep fixing until the output matches. "
    "If it states a source-size cap, check wc -c and shrink until it fits. "
    ...
    "every one, and write the scored file before wrap-up. If the instruction "
    "asks for player moves or typed commands from a video, write those "
    "command lines, not a dump of on-screen narration. If the program "
    ...
    "browser can still open it. A Connection refused from that browser "
