...
| L2 failed-prev | `workflow_dispatch` on `Evals weekly` with split `failed-prev` | `steerable` | previous catalog zeros + install errors + 1→0 flips (~41 ids, 4 shards) |
...
Harness iteration uses split `failed-prev` (41 ids, 4 shards) so a headless change is not gated on rerunning all 89. That Mean is not the score of record; catalog 89 is.
