...
describe('LocalStore / 消息写入与读取', () => {
  ...
  it('addMessage 刷新所属会话的 updated_at', () => {
    ...
    store.addMessage(chat.id, 'user', 'hi');
    expect(store.getChat(chat.id)?.updatedAt).toBe('2026-01-01T00:00:09.000Z');
  });

  it('向不存在的会话写消息被外键约束拒绝', () => {
    const { store } = createTestStore(LocalStore);
    // foreign_keys = ON 是构造时显式打开的；孤儿消息必须炸在写入侧。
    expect(() => store.addMessage('ghost', 'user', 'hi')).toThrow(/FOREIGN KEY/);
  });

  it('getMessage 按 chat 隔离：同 id 在别的会话不可见', () => {
    const { store } = createTestStore(LocalStore);
    const a = store.createChat('a');
    const b = store.createChat('b');
    const msg = store.addMessage(a.id, 'user', 'hi');
    expect(store.getMessage(b.id, msg.id)).toBeNull();
    expect(store.getMessage(a.id, 'ghost')).toBeNull();
  });
});

describe('LocalStore / listMessages 排序与截断', () => {
  it('新→旧返回（MESSAGE_ORDER_DESC），调用方自行反转成 transcript 序', () => {
    vi.useFakeTimers();
    const { store } = createTestStore(LocalStore);
    const chat = store.createChat('x');
    const ids: string[] = [];
  ...
  it('同毫秒连写靠 rowid 决胜：读出顺序依然稳定', () => {
    ...
    vi.setSystemTime('2026-01-01T00:00:00.000Z');
    const ids = [0, 1, 2].map((i) => store.addMessage(chat.id, 'user', `m${i}`).id);
    expect(store.listMessages(chat.id).map((m) => m.id)).toEqual([ids[2], ids[1], ids[0]]);
  });

  it('limit 截断与 clamp：NaN 回默认 200，0 夹到 1', () => {
    const { store } = createTestStore(LocalStore);
    const chat = store.createChat('x');
    for (let i = 0; i < 3; i += 1) store.addMessage(chat.id, 'user', `m${i}`);
    expect(store.listMessages(chat.id, 2)).toHaveLength(2);
    expect(store.listMessages(chat.id, Number.NaN)).toHaveLength(3);
    expect(store.listMessages(chat.id, 0)).toHaveLength(1);
    expect(store.listMessages('ghost')).toEqual([]);
  });
});

describe('LocalStore / deleteMessagesFrom（regenerate 截断）', () => {
  it('删掉目标消息及其后的全部消息，之前的保留', () => {
    vi.useFakeTimers();
    const { store } = createTestStore(LocalStore);
    const chat = store.createChat('x');
    const ids: string[] = [];
    for (let i = 0; i < 5; i += 1) {
      vi.setSystemTime(`2026-01-01T00:00:0${i}.000Z`);
      ids.push(store.addMessage(chat.id, i % 2 ? 'assistant' : 'user', `m${i}`).id);
    }
    // 从第 3 条（ids[2]）起删：被重新生成的回复及其后全部截掉。
    expect(store.deleteMessagesFrom(chat.id, ids[2])).toBe(3);
    expect(store.listMessages(chat.id).map((m) => m.id)).toEqual([ids[1], ids[0]]);
  });

  it('未知 messageId 不删任何行；不波及其他会话', () => {
    const { store } = createTestStore(LocalStore);
    const a = store.createChat('a');
    const b = store.createChat('b');
    const ma = store.addMessage(a.id, 'user', 'a1');
    store.addMessage(b.id, 'user', 'b1');
    // 子查询对未知 id 返回 NULL，整条件不成立。
    expect(store.deleteMessagesFrom(a.id, 'ghost')).toBe(0);
    // b 会话里的同 id 消息不构成匹配（子查询带 chat_id 条件）。
    expect(store.deleteMessagesFrom(b.id, ma.id)).toBe(0);
    expect(store.listMessages(a.id)).toHaveLength(1);
    expect(store.listMessages(b.id)).toHaveLength(1);
  });
});

describe('LocalStore / replaceChatMessages（分支投影）', () => {
  it('整体替换消息列表，合成时间戳保持输入序，会话 updated_at 刷新', () => {
    vi.useFakeTimers();
    const { store } = createTestStore(LocalStore);
    vi.setSystemTime('2026-01-01T00:00:00.000Z');
    const chat = store.createChat('x');
    store.addMessage(chat.id, 'user', '旧消息1');
    store.addMessage(chat.id, 'assistant', '旧消息2');

    vi.setSystemTime('2026-01-01T01:00:00.000Z');
    const base = Date.now();
    store.replaceChatMessages(chat.id, [
    ...
    expect(store.getChat(chat.id)?.updatedAt).toBe(new Date(base + 3).toISOString());
  });
  ...
  it('空数组投影 = 清空消息', () => {
    ...
    expect(store.chatHasMessages(chat.id)).toBe(false);
  });
});
