...
| Live-lock pre-wrap + keep-required | **Yes, next SHA** | Same env. Count `empty_round` while `writes == 0`; after WRITE_NOW keep `tool_choice=required` and wrap-up tools until a write. Mechanism: `circuit-fibsqrt` |
