...
**6 道文件缺失的真实原因仍未知。** 原推断"检测器漏了路径"已被证伪——路径一直都检测得到，`_required` 一直包含它们，门禁本该拒绝完成（最多 32 次重试）。所以要么是切流杀在门禁生效之前（这 6 道里 3 道确实被切流卡住，但切流已默认关闭），要么是 trial 根本没走到 `before_completion`（硬超时 / 轮次耗尽）。下一步应查这 6 道在 0.8202 那跑的 `headless.log` 里到底是怎么结束的，而不是继续改检测器。
...
- [ ] **2.5.9** 查那 6 道"文件缺失"红题在 0.8202 跑里的真实结束方式（`_required` 一直包含那些路径，门禁却没拦住）。先定位是硬超时、轮次耗尽还是门禁被绕过，再决定改什么
