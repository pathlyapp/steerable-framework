...
- [ ] **2.5.7** 测 `8acc022`（命名输出检测）：catalog-89 [33514869908](https://github.com/pathlyapp/steerable-framework/actions/runs/33514869908)，相对 0.8202 只差这一个变量
- [ ] **2.5.8** 持久性指令 + 门禁判据打包成一个 arm 测。单次全量分辨不了这个量级，按本文档自己的功效表要走配对 A/B；`flaky` 的 25 题是按切流那一轮的翻面集选的，0.8202 之后翻面集已经变了，要先用三次全量的同题对照重建
