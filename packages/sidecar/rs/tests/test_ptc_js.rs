...
#[test]
fn run_js_is_listed_when_enabled_and_returns_a_value() {
    ...
    let names: Vec<&str> = listed["result"]
        ...
        .collect();
    assert!(names.contains(&"run_js"), "{listed}");
    assert!(names.contains(&"wait_js"), "{listed}");
    ...
    assert_eq!(
        reply["result"]["data"]["status"],
        json!("completed"),
        "{reply}"
    );
    assert_eq!(reply["result"]["data"]["value"]["n"], json!(42), "{reply}");
    assert_eq!(reply["result"]["data"]["output"], json!(["hello"]));
    let logs = reply["result"]["data"]["logs"]
        .as_array()
        .cloned()
        .unwrap_or_default();
    assert!(
        logs.iter()
            .any(|line| line.as_str().unwrap_or("").contains("dbg")),
        "{reply}"
    );
...
#[test]
fn run_js_refuses_empty_source() {
    let mut child = spawn_sidecar();
    read_ready(&mut child);
    let stdout = child.stdout.take().expect("stdout");
    let mut stdout = std::io::BufReader::new(stdout);
