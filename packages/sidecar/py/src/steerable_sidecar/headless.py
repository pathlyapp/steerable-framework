...
_SYSTEM = (
    ...
    "/usr/bin if it landed in /usr/sbin. A scored script must run under "
    "the grader's interpreter with its existing modules: prefer stdlib "
    "and system CLIs over pip (a pip install lands in one interpreter "
    "only), and verify with bare `python x.py`, not only `python3`. "
    "Print dates as YYYY-MM-DD when a "
    ...
    "carving. When decoding binary records, sanity-check values against "
    "their siblings: a huge integer among small ones is a float decoded "
    "as an int — recheck type tag, width, endianness.\n"
