...
and install matching manylinux or musllinux `cp310-abi3` wheels and run with
...
| **Steerable** | **80.7% ±2.9** | 6 | 271.7 M | ~$10.50 | ~$0.146 |
...
Fill-in GitHub Actions at `19213d7`: Claude Code × DS [34684494958](https://github.com/pathlyapp/steerable-framework/actions/runs/34684494958), Pi × Qwen [34684497256](https://github.com/pathlyapp/steerable-framework/actions/runs/34684497256), Codex × GLM [34689464813](https://github.com/pathlyapp/steerable-framework/actions/runs/34689464813), Codex × DS [34689466743](https://github.com/pathlyapp/steerable-framework/actions/runs/34689466743), Codex × Qwen [34689468681](https://github.com/pathlyapp/steerable-framework/actions/runs/34689468681). DSH catalog at `c576a88`: GLM [34848045490](https://github.com/pathlyapp/steerable-framework/actions/runs/34848045490), DS0731 [34848052842](https://github.com/pathlyapp/steerable-framework/actions/runs/34848052842), Qwen [34848057723](https://github.com/pathlyapp/steerable-framework/actions/runs/34848057723).
...
Six catalog runs at that commit (GHA [34031313764](https://github.com/pathlyapp/steerable-framework/actions/runs/34031313764), [34031319806](https://github.com/pathlyapp/steerable-framework/actions/runs/34031319806), [34040053173](https://github.com/pathlyapp/steerable-framework/actions/runs/34040053173), [34122470060](https://github.com/pathlyapp/steerable-framework/actions/runs/34122470060), [34122485974](https://github.com/pathlyapp/steerable-framework/actions/runs/34122485974), [34139665379](https://github.com/pathlyapp/steerable-framework/actions/runs/34139665379); tool: `python -m evals.stratify_catalog --root <downloaded-shards>`) split the 89 ids as:

- **55 stable green** (6/6) — includes `qemu-alpine-ssh` and `qemu-startup`, converted from deterministic setup crashes by the EOL-apt fix
- **7 stable red** (0/6): `extract-moves-from-video`, `filter-js-from-html`, `gcode-to-text`, `make-doom-for-mips`, `protein-assembly`, `regex-chess`, `video-processing`
- **27 flaky** (listed under `splits.flaky` in `evals/suite.yaml`, with x/6 comments): 3×1/6, 4×2/6, 1×3/6, 8×4/6, 11×5/6

Expected passes = 55 + 16.83 = 71.83 (80.71%), matching the observed six-run mean. The 70/71/77/71/71/71 spread is the flaky layer's coin tosses, so the floor misses 72 while the mean clears it. Doubling the sample moved ten three-run "stable green" tasks into flaky (they were coin tosses the small sample hid) and pulled `pytorch-model-cli` (2/6) and `winning-avg-corewars` (1/6) up from stable red; `loss-24` is now `loss-34` (27 flaky + 7 red). Rebuild them only after a new multi-run catalog, not from a single dispatch. Mechanism labels for the 34 losses: `evals/notes/loss-taxonomy.md`.

**Context length defeats zero failures.** Across all 103 failed trials in the six runs, none peaked within 90% of the model's 1,048,576-token window — in fact none reached 50% (max 473,325 = 45.1%, median 149,117 = 14.2%; per-trial `peak_context_tokens` from the `STEERABLE_RUN_SUMMARY` log line). Every loss is capability-side, so the roadmap invests in tools and mechanisms, not in bigger-window plumbing.
