...
// ─── Built-in patterns ──────────────────────────────────────────────
// Merged from local-actions.ts (web, severity=warning) and
// local-executor.ts (desktop, severity=critical).
...
export function classifyShellCommand(command: string, config?: CommandSafetyConfig | null): ShellCommandClassification {
  ...
  const disabled = new Set(config?.disabledPatternIds ?? []);
  const matches = BUILTIN_PATTERNS
