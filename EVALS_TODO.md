...
- [x] **2.5.22** 跨厂商 catalog 的 `STEERABLE_REASONING_EFFORT` 是空转的。`evals-weekly.yml` 对 `*qwen*` 设 `medium`（`f3d4495`），但 `clamp_reasoning_effort` 拿 `openai/qwen/qwen3.8-27b` 查 `model_catalog` 查不到 —— 目录里只有 `openrouter/qwen/qwen3.8-27b`、`aiand/qwen/qwen3.8-27b` 等前缀，网关自己的 `openai/` 命名空间一条都没有。`reasoning_levels` 为空 → clamp 返回 `None` → `openai_compat` 连 `reasoning_effort` 带 `reasoning` 两个字段都不发。**运行 34181536737 / 34168003658 的 Qwen 75.3% 因此跑在未知的服务端默认档上**，对不齐 AA 那次 xhigh。~~要么给网关前缀补目录条目，要么让 clamp 查不到时报错而不是静默丢弃~~ **已修（gateway-catalog 分支）**：`model_resolve.resolve_leaf_cross_provider` 新增跨 provider 末段精确 join，`openai/qwen/qwen3.8-27b` 现在命中 `qwen3.8-27b` 的目录条目并与 legacy 表取并集（`{low, medium, xhigh}`——上游 models.dev 现行快照给 qwen3.8-27b 标了 xhigh，即 AA 那次用的档位），`medium` 会真正发到线上；同时 `clamp_reasoning_effort(strict=True)` 在请求路径上对未知模型/无档位/档位不支持/档位拼写错误全部抛 `ReasoningEffortUnsupported`（`openai_compat` 转成 `LLMError(invalid_request)`），静默丢弃这条路径已删除。overlay 方案不可行——它是生成期 corrections-only，拒绝添加快照里不存在的模型。注意：修复后历史 qwen 运行（未知档位）与修复后运行（medium）不可直接对比。
...
- [x] **2.5.23** qwen cheap-12 两个稳定红题的机制与 prompt 修复。运行 34181536737（medium）与 34321839558（xhigh）各 9/11，挂的是同两题，提档不改变结果——机制性失败，非采样噪声。① `openssl-selfsigned-cert`：agent 写的 `/app/check_cert.py` 依赖 pip 装的 `cryptography`，自己用 `python3` 验证通过，但 grader 用 `python` 调它（另一解释器，无此模块）→ `ModuleNotFoundError`；agent 自己想过"openssl subprocess 更稳"但没采纳。② `sqlite-db-truncate`：8 行答案需对 7 行，agent 恢复出全部 8 个 word，但把 SQLite serial type 7 的 float64 按 int64 读，`99.99`→`4636736587667194511`、`0.5`→`4602678819172646912`，整数值的 6 行靠 Python `10 == 10.0` 侥幸匹配，恰差一行。修复落在 `_SYSTEM`（prompt 预算 7000 字符硬上限内）："What is scored" 加评分解释器条款（被评分的脚本必须跑在 grader 调用的解释器上、用其已有模块，优先 stdlib/系统 CLI，用裸 `python x.py` 验证而非只 `python3`）；"Domain notes" 加二进制解码合理性条款（同族值里冒出一个巨大整数 = float 被当 int 解，回查 type tag/宽度/端序）。两题工具链（bash/read/write/edit）行为都正确，无工具侧修复。
